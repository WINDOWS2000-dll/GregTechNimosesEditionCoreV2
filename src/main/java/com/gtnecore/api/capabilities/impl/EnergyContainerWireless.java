    ...
    public void update() {
        ...
        if (!this.metaTileEntity.getWorld().isRemote) {
            ...
            if (this.metaTileEntity.getOwner() != null) {
                ...
                WirelessEUNetworkNode node = dataBase
                        .getNetWork(((MetaTileEntityWirelessEnergyHatch) metaTileEntity).WirelessID);

                if (node == null) {
                    WirelessEUNetworkManager.INSTANCE.createNetwork(world, metaTileEntity.getOwner(),
                            "wirelessNetwork");
                ...
                if (this.getInputVoltage() == 0) {
                    if (this.energyStored > 0) {
                        ...
                        if (node != null) {
                            long added = WirelessEUNetworkManager.INSTANCE.transferEnergy(world, node.getNetworkID(),
                                    b1);
